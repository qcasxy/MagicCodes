...
function getLoginToken() {
  return new Promise(resolve => {
    const options = {
      ...
      "body": `app_name=com.xiaomi.hm.health&app_version=6.3.1&code=${$.access}&country_code=CN&device_id=2C8B4939-0CCD-4E94-8CBA-CB8EA6E613A1&device_model=phone&grant_type=access_token&third_name=huami_phone`,
      "headers": headers
    }
    $.post(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          login_token = loginInfo.token_info.login_token
          if (login_token) {
            $.setdata(login_token, 'xmSportsToken');
            start()
          }else {
            console.log(`登陆失败，未能获取Login Token`);
          }
        }
      } catch (e) {
        $.logErr(e, resp)
